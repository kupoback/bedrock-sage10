...
/**
 * Adds the favicons uploaded in the images/favicons folder that
 * was generated by RealFaviconGenerator
 *
 * @link https://realfavicongenerator.net/
 */
add_action('sage_favicons', function () {
    printf(
        '%s',
        collect(
            [
            ...
            ->map(function ($favicon) {
                if (($favicon->href ?? false) && !$favicon->href->exists()) {
                    return '';
                }

                $contents = collect($favicon)
                    ->map(fn ($value, $key) => "$key='$value'");
                if ($favicon->meta ?? false) {
                    return sprintf(
                        '<meta %s>',
                        $contents->filter(fn ($value, $key) => $key !== 'meta')
                                 ->implode(' '),
                    );
                }
