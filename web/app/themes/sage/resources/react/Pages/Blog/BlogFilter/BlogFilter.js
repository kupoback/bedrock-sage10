...
function BlogFilter({}) {
	const {filterLabel, filterSubmit, labels, searchLabel, searchPlaceholder, taxonomies} = RBLOG;
		...
		<div className="rounded border border-gray-100 p-4 blog-listing__filters">
			...
			<div className="pb-2 blog-listing__filters-submit">
				<Button btnClass="group flex items-center justify-between rounded-lg border border-gray-100 px-5 py-3 transition-colors hover:border-yellow-500 hover:bg-yellow-500 focus:outline-none focus:ring"
				        btnType="submit"
				        content={<span className="font-medium text-white transition-colors group-hover:text-gray-900 group-active:text-gray-900">{filterSubmit}</span>}
			        />
			</div>
		</div>
	);
